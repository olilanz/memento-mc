...
class WorldMementoMap {
     ...
     * - Within the chosen region, prefer anchor chunks on the local (1,1) modulo-3 grid (local
     * coords relative to region origin). This reduces cross-region spillover from side-effect chunk
     * loads.
     * - Expose only missing chunks on the modulo-3 grid anchored by the chosen anchor.
     * - If no missing chunk exists on the preferred (1,1) grid, fall back to anchoring on the next
     * missing chunk in the region (same algorithm, different residue).
     *
    ...
    fun missingSignals(limit: Int): List<ChunkKey> {
        ...
        // 1) Choose the next region to work on: the smallest (world, regionX, regionZ) that still
        // has missing chunks.
        ...
        for ((k, v) in records.entries) {
            ...
            if (fallbackAnchor == null ||
                            k.chunkX < fallbackAnchor.chunkX ||
                            (k.chunkX == fallbackAnchor.chunkX && k.chunkZ < fallbackAnchor.chunkZ)
            ) {
                fallbackAnchor = k
            }
            ...
            if (lx == 1 && lz == 1) {
                if (preferredAnchor == null ||
                                k.chunkX < preferredAnchor.chunkX ||
                                (k.chunkX == preferredAnchor.chunkX &&
                                        k.chunkZ < preferredAnchor.chunkZ)
                ) {
                    preferredAnchor = k
                }
            }
        ...
        return records.entries
                ...
                .map { it.key }
                .filter {
                    it.world.value.toString() == region.worldKey &&
                            it.regionX == region.regionX &&
                            it.regionZ == region.regionZ
                }
                .filter {
                    localMod3(it.chunkX, it.regionX) == anchorRx &&
                            localMod3(it.chunkZ, it.regionZ) == anchorRz
                }
                .sortedWith(
                        compareBy(
