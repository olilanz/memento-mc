...
The Witherstone itself does not load chunks, does not trigger regeneration, and does not bypass engine constraints. It establishes renewal intent that later participates in election.
...
Renewal election derivation is owned on the renewal side and must use `StoneMapService` for dominant-stone lookup rather than re-deriving stone influence internally.
...
Cadence constants and phase offsets are centralized in `MementoConstants`.
...
* Non-HIGH phase staggering policy is implementation-owned in pulse infrastructure and guarded by runtime validation.
...
Component-local algorithm, batching, retry, schema, and error-handling policies are documented in owner class headers in code.

`ARCHITECTURE.md` keeps cross-component doctrine only: authority boundaries, semantic layering, thread/event ownership, and anti-orchestrator constraints. ADRs remain the historical rationale record.

Moving local policy from this document into class headers does not reduce force; it relocates ownership to the component that enforces the policy at runtime.
